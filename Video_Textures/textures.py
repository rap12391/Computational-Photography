...
def computeSimilarityMetric(video_volume):
    ...
    """
    similarity_matrix = np.zeros((video_volume.shape[0], video_volume.shape[0]),dtype=np.float64)
    for i in range(video_volume.shape[0]):
        for j in range(video_volume.shape[0]):
            similarity_matrix[i,j] = np.sqrt(np.sum(np.square(np.float64(video_volume[i])-np.float64(video_volume[j]))))
    avg_val = np.average(similarity_matrix)
    normalized = similarity_matrix/avg_val
    return normalized
...
def transitionDifference(similarity):
    ...
    """
    binomial_array = binomialFilter5()
    binomial_kernel = np.diag(binomial_array)
    diff_matrix = cv2.filter2D(similarity,-1,binomial_kernel)[2:-2,2:-2]
    return diff_matrix
...
def findBiggestLoop(transition_diff, alpha):
    ...
    **************************************************************************
    ...
    """
    start, end = 0, 0
    max_score = -transition_diff[0,0]
    for i in range(transition_diff.shape[0]):
        for j in range(transition_diff.shape[0]):
            score = alpha*(j-i) -transition_diff[j,i]
            if score > max_score:
                max_score = score
                start = i
